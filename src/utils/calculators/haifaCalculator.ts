...
export interface HaifaCalculatorInput {
...
export interface HaifaCalculatorResult {
	bagrutAverage: number;
	standardBagrutScore: number; // BT - ממוצע תקן בגרות (סולם פסיכומטרי)
	generalSekem: number; // סכם רשמי אוניברסיטת חיפה (סולם 200-800)
...
export interface OptimalHaifaBagrutResult {
	average: number;
	optimalUnits: number;
	totalUnits: number;
...
/**
 * Returns University of Haifa official bonus for a subject:
 ...
 */
export function getHaifaSubjectBonus(sub: SubjectInput): number {
...
/**
 * Checks if a subject is mandatory according to University of Haifa matriculation requirements:
 * Haifa keeps all official mandatory subjects (Math, English, Civics, Hebrew, History, Literature, Bible).
 */
...
/**
 * Calculates University of Haifa Optimal Bagrut Average:
 * Drops additional electives that drag down the average, maintaining mandatory subjects and >= 20 total units.
 */
export function calculateOptimalHaifaBagrut(subjects: SubjectInput[]): OptimalHaifaBagrutResult {
	...
	if (droppableSubs.length === 0) {
		...
		for (const s of activeSubs) {
			totalScore += (s.grade + getHaifaSubjectBonus(s)) * s.units;
		}
...
/**
 * Calculates University of Haifa Official Standard Bagrut Score (BT) & Sekem:
 * Official published formula:
 * BT = Bagrut_Average * 10 - 330
 * Sekem = 0.5 * BT + 0.5 * Psychometric = 5.0 * Bagrut_Average + 0.5 * Psychometric - 165
 * Scale: 200 - 800
 */
export function calculateHaifaSekem(
	bagrutAverage: number,
	psychometric: number
): { sekem: number; bt: number } {
	if (bagrutAverage <= 0 || psychometric <= 0) return { sekem: 0, bt: 0 };
...
/**
 * Calculates full University of Haifa Admission Evaluation
 */
export function calculateHaifaAdmission(input: HaifaCalculatorInput): HaifaCalculatorResult {
	const optimal = calculateOptimalHaifaBagrut(input.bagrutSubjects);
	const psych = input.psychometricGeneral;
	const quant =
		input.psychometricQuant && input.psychometricQuant > 0 ? input.psychometricQuant : psych;

	const { sekem: generalSekem, bt: standardBagrutScore } = calculateHaifaSekem(
		optimal.average,
		psych
	);
	const { sekem: engineeringSekem } = calculateHaifaSekem(optimal.average, quant);
